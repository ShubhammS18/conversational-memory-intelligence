...
- [`M10-fixed-workload-evaluation-bindings`](M10-fixed-workload-evaluation-bindings.md) — fixed original fixtures, explicit evaluation-only adaptation, frozen real-component execution, hard safety gates, truthful case verdicts, deterministic private reporting, baseline comparison, and CLI exit rules; effective only after approval and application.
- [`M10-freeze-boundary-exceptions`](M10-freeze-boundary-exceptions.md) — exactly five M10 governance paths; future implementation remains within the locked evaluation, regression, and entrypoint boundary and requires separate BUILD authorization.
